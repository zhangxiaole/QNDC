...
dc.razor_product p,
       dc.razor_channel_product cp,
       dc.razor_channel c,
       dc.razor_clientdata cd,
       dc.razor_product_category pc,
       dc.razor_platform pf
set 
...
cd.productkey,
                ...
                cp.user_id,
                pf.name
from  dc.razor_product p inner join
       dc.razor_channel_product cp on p.id = cp.product_id inner join
       dc.razor_channel c on cp.channel_id = c.channel_id inner join
       dc.razor_product_category pc on p.category = pc.id inner join
       dc.razor_platform pf on c.platform = pf.id inner join (select distinct
       productkey,version from dc.razor_clientdata) cd on cp.productkey = cd.productkey  
...
from   dc.razor_clientusinglog f,dc.razor_product p,dc.razor_channel_product cp
where  
...
/* dim event */
set s = now();
update razor_dim_event e,dc.razor_event_defination d
...
from   dc.razor_event_defination f
where  not exists (select *
